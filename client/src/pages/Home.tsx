...
export default function Home() {
  ...
  // Load progress
  useEffect(() => {
    const saved = localStorage.getItem("one-stroke-progress");
    if (saved) {
  ...
  const handleSubmitScore = async () => {
    ...
    try {
      await submitScore.mutateAsync({
        playerName,
        levelReached: 10 // Completed game
      });
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col relative overflow-hidden">
          ...
          <div className="max-w-4xl w-full flex flex-col items-center gap-12 py-12">
            <div className="text-center space-y-4">
              <h2 className="text-5xl md:text-7xl font-display font-black tracking-tighter text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.3)]">
                NETWORK <span className="text-primary text-glow">PROTOCOL</span>
              </h2>
        ...
        <DialogContent className="bg-slate-950/90 border-slate-800 text-white backdrop-blur-xl max-w-md">
          <DialogHeader>
            <DialogTitle className="text-2xl font-display text-primary flex items-center gap-2">
              <Trophy className="w-6 h-6" /> GLOBAL RANKING
            </DialogTitle>
      ...
      <Dialog open={showVictoryModal} onOpenChange={setShowVictoryModal}>
        <DialogContent className="bg-slate-950/95 border-primary/50 text-white backdrop-blur-xl">
          <DialogHeader>
            <DialogTitle className="text-3xl font-display text-center text-transparent bg-clip-text bg-gradient-to-r from-primary to-purple-500 pb-2">
              PROTOCOL COMPLETE
            </DialogTitle>
          ...
          <div className="py-6 space-y-4">
            <div className="p-4 bg-primary/10 border border-primary/20 rounded-lg text-center">
              <p className="text-sm text-primary uppercase tracking-widest mb-1">Final Status</p>
              <p className="text-4xl font-black font-display text-white">100%</p>
            </div>
